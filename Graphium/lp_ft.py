...
def train(args, epoch, model, output_layer, device, loader, optimizer):
    model.train()
    epoch_iter = tqdm(loader, desc="Iteration")
    for step, batch in enumerate(epoch_iter):
        batch = batch.to(device)
        h = global_mean_pool(model(batch).feat, batch.batch)
        pred = output_layer(h)
...
def eval(args, model, output_layer, device, loader):
    model.eval()
    y_true = []
    y_scores = []
...
def linear_probing(args, model, output_layer, device, train_loader, optimizer_cls, val_loader, test_loader, writer):
    train_acc_list = []
    val_acc_list = []
    test_acc_list = []
    best_val_acc = 100 if args.regression else -1
    best_cls = output_layer.state_dict()
    best_func = min if args.regression else max

    for epoch in range(1, args.epochs+1):
        print("====LP epoch " + str(epoch))
        
        train(args, epoch, model, output_layer, device, train_loader, optimizer_cls)

        print("====LP Evaluation")
        if args.eval_train:
            train_acc = eval(args, model, output_layer, device, train_loader)
        else:
            print("omit the training accuracy computation")
            train_acc = 0
        val_acc = eval(args, model, output_layer, device, val_loader)
        test_acc = eval(args, model, output_layer, device, test_loader)
        ...
        bol_update = (val_acc < best_val_acc) if args.regression else (val_acc > best_val_acc)
...
def main():
    ...
    for seed in allseed:
        ...
        #set up model
        # model = graphium.trainer.PredictorModule.load_pretrained_model('models_checkpoints/neurips2023-small-gin/2024-07-27_15-42-01_20240727_154201/neurips2023_small_data_gin_20240727_154201.ckpt').model
        model = graphium.trainer.PredictorModule.load_pretrained_model('model_checkpoints/large-dataset/gin/2024-12-31_06-58-55_20241231_065855/large_data_gin_20241231_065855.ckpt').model
        model.task_heads = None
        ...
        # params_list = list(itertools.chain(*tune_params_dict['encoder']))
        optimizer = optim.Adam(list(model.gnn.parameters()) + list(output_layer.parameters()), lr=args.lr, weight_decay=args.decay)
        optimizer_cls = optim.Adam(output_layer.parameters(), lr=args.lr, weight_decay=args.decay)

        train_acc_list = []
        val_acc_list = []
        test_acc_list = []

        args.filename = "LP-FT_fewshot" + "_" + args.dataset + "_" + args.split + "_" + "Fewshot_" + str(args.fewshot) + "_" + str(args.fewshot_num)

        if not args.filename == "":
            fname = 'runs/LP-FT_fewshot_runseed' + str(args.runseed) + '/' + args.filename
        ...
        best_cls = linear_probing(args, model, output_layer, device, train_loader, optimizer_cls, val_loader, test_loader, writer)
        output_layer.load_state_dict(best_cls)

        for epoch in range(args.epochs+1, 2*args.epochs+1):
            print("====epoch " + str(epoch))
            
            train(args, epoch, model, output_layer, device, train_loader, optimizer)
        ...
        exp_path = os.getcwd() + '/sup_largemix_results/{}/'.format(args.dataset)
        ...
        logs = 'Dataset:{}, Split:{}, Fewshot_{}_{}, Seed:{}, LP-FT, Best Epoch:{}, Best Acc:{:.5f}'.format(args.dataset, args.split, args.fewshot, args.fewshot_num, args.runseed, val_acc_list.index(best_func(val_acc_list)), test_acc_list[val_acc_list.index(best_func(val_acc_list))])
    ...
    logs = 'Dataset:{}, Split:{}, Fewshot_{}_{}, All seed, LP-FT, Best Acc:{:.5f}, std: {:.5f}'.format(args.dataset, args.split, args.fewshot, args.fewshot_num, np.mean(score_list), np.std(score_list))
    with open(exp_path + '{}_log.csv'.format(args.dataset),'a+') as f:
        f.write('\n')
        f.write(logs)
