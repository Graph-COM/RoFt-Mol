...
class MoleculeDataset(InMemoryDataset):
    def __init__(self,
        ...
        if not empty:
            self.data, self.slices = torch.load(self.processed_paths[0])
            # if self.dataset not in ['malaria', 'cep', 'mpbp', 'bbbp', 'bace']:
            #     self.data = bump(self.data)
            # # self.slices = bump(self.slices)
        print("h")
    ...
    def process(self):
        ...
        elif self.dataset == 'mpbp':
            smiles_list, rdkit_mol_objs, labels = \
                _load_mpbp_dataset(self.raw_paths[0])
            for i in range(len(smiles_list)):
                print(i)
                rdkit_mol = rdkit_mol_objs[i]
                # # convert aromatic bonds to double bonds
...
def _load_mpbp_dataset(input_path):
    """
