...
def main():
    ...
    for seed in allseed:
        ...
        train_acc = eval(args, pt_model, ft_model, output_layer, device, train_loader, args.alpha)
        val_acc = eval(args, pt_model, ft_model, output_layer, device, val_loader, args.alpha)
        test_acc = eval(args, pt_model, ft_model, output_layer, device, test_loader, args.alpha)
        ...
        args.filename = "FTMAP" + "_" + args.dataset + "_" + args.split + "_" + "Fewshot_" + str(args.fewshot) + "_" + str(args.fewshot_num) + "alpha_" + str(args.alpha)
        ...
        logs = 'Dataset:{}, Split:{}, Fewshot_{}_{}, Seed:{}, FTMAP:{}, Best Epoch:{}, Best Acc:{:.5f}'.format(args.dataset, args.split, args.fewshot, args.fewshot_num, args.runseed, args.alpha, val_acc_list.index(best_func(val_acc_list)), test_acc_list[val_acc_list.index(best_func(val_acc_list))])
    ...
    logs = 'Dataset:{}, Split:{}, Fewshot_{}_{}, All seed, FTMAP:{}, Best Acc:{:.5f}, std: {:.5f}'.format(args.dataset,args.split, args.fewshot, args.fewshot_num, args.alpha, np.mean(score_list), np.std(score_list))
    with open(exp_path + '{}_log.csv'.format(args.dataset),'a+') as f:
        f.write('\n')
        f.write(logs)
