...
class CustomLogger(Logger):
    ...
    def classification_binary(self):
        ...
        if true.shape[0] < 1e7:  # AUROC computation for very large datasets is too slow.
            ...
            # print(true==1)
            # print(torch.sum(true==1))
            # print(torch.sum(true==-1))
            is_valid_bool = False
            if torch.sum(true == 1) > 0 and torch.sum(true == -1) > 0:
                is_valid = true**2 > 0
                is_valid_bool = True
            
            if is_valid_bool:
                auroc_score = auroc(pred_score[is_valid].to(torch.device(cfg.accelerator)),
                                    ((true[is_valid]+1)/2).to(torch.device(cfg.accelerator)),
                                    task='binary')
            else:
                auroc_score=0
            if self.test_scores:
...
def create_logger():
    ...
    for i, dataset in enumerate(range(3)):
