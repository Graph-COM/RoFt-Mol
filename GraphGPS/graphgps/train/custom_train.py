...
from graphgps.loss.reg_loss import l2_sp, feature_map, bss


def train_epoch(logger, loader, model, model_pt, optimizer, scheduler, batch_accumulation, reg_coef):
    model.train()
    model_pt.eval()
    optimizer.zero_grad()
    ...
    for iter, batch in enumerate(loader):
        ...
        true = true.view(pred.shape).to(torch.float64)
        ...
        if 'l2_sp' in cfg.model.loss_fun:
            l2_sp_loss = l2_sp(model, model_pt)
            loss = loss + reg_coef*l2_sp_loss
        elif 'feature_map' in cfg.model.loss_fun:
            feature_map_loss = feature_map(model, model_pt, batch_clone)
            loss = loss + reg_coef*feature_map_loss
        elif 'bss' in cfg.model.loss_fun:
            bss_loss = bss(batch.x)
            loss = loss + reg_coef*bss_loss
        loss.backward()
...
def custom_train(loggers, loaders, model, model_pt, reg_coef, optimizer, scheduler):
    """
    ...
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        ...
        train_epoch(loggers[0], loaders[0], model, model_pt, optimizer, scheduler,
                    cfg.optim.batch_accumulation, reg_coef)
