...
#     def __init__(self, embed_dim, num_heads, dropout=0.0, lora_r=8, lora_alpha=32, lora_dropout=0.1):
#         super().__init__()
...
#         )
#         self.mha = get_peft_model(self.mha, lora_config)

#     def forward(self, x, key_padding_mask=None, attn_mask=None):
#         return self.mha(x, x, x, key_padding_mask=key_padding_mask, attn_mask=attn_mask)[0]
    
from torch import nn
from peft.tuners.lora.layer import Linear as LoraLayer

class LoRAMultiheadAttention(nn.Module):
    def __init__(self, dim_h, num_heads, attn_dropout,
        ...
        # 1) Create your base q/k/v/out projection layers:
        base_q = nn.Linear(dim_h, dim_h, bias=False)
        base_k = nn.Linear(dim_h, dim_h, bias=False)
        base_v = nn.Linear(dim_h, dim_h, bias=False)
        base_out = nn.Linear(dim_h, dim_h, bias=True)

        # 2) Wrap ONLY q and v with LoRA adapters:
        self.q_proj = LoraLayer(
            base_layer=base_q,
            adapter_name="q_proj_lora",
            r=lora_r,
            lora_alpha=lora_alpha,
            lora_dropout=lora_dropout,
        ...
        self.k_proj = base_k   # no LoRA on k
        self.v_proj = LoraLayer(
            base_layer=base_v,
            adapter_name="v_proj_lora",
            r=lora_r,
            lora_alpha=lora_alpha,
            lora_dropout=lora_dropout,
            fan_in_fan_out=False,
    ...
    def forward(self, x, key_padding_mask=None):
        B, N, D = x.size()
        # project
        q = self.q_proj(x)   # includes LoRA
        k = self.k_proj(x)
        v = self.v_proj(x)   # includes LoRA

        # reshape to (B, heads, N, D/head)
...
class GPSLayer_lora(nn.Module):
    def __init__(self, dim_h, local_gnn_type, global_model_type, num_heads, act='relu',
        ...
        if global_model_type == 'None':
            ...
            # base_attn = torch.nn.MultiheadAttention(dim_h, num_heads,
            ...
            #     target_modules=["in_proj_weight", "out_proj"]
            # )
            # self.self_attn = get_peft_model(base_attn, config)
            ...
            self.self_attn = SelfAttention(dim=dim_h, heads=num_heads,
                                           dropout=self.attn_dropout, causal=False)
        elif global_model_type == 'BigBird':
            bigbird_cfg.dim_hidden = dim_h
            bigbird_cfg.n_heads = num_heads
    ...
    # (The forward pass remains the same and uses self.self_attn)
    def forward(self, batch):
        h = batch.x
        h_in1 = h  # for first residual connection
        ...
        if self.self_attn is not None:
            ...
            if self.global_model_type == 'Transformer':
                ...
                h_attn_dense = self.self_attn(h_dense, key_padding_mask=~mask)
                h_attn = h_attn_dense[mask]
            elif self.global_model_type == 'BiasedTransformer':
                # Use Graphormer-like conditioning, requires `batch.attn_bias`.
                h_attn = self.self_attn(h_dense, batch.attn_bias, ~mask)[mask]
            elif self.global_model_type == 'Performer':
                h_attn = self.self_attn(h_dense, mask=mask)[mask]
            elif self.global_model_type == 'BigBird':
                h_attn = self.self_attn(h_dense, attention_mask=mask)
    ...
    def extra_repr(self):
        s = f'summary: dim_h={self.dim_h}, ' \
            f'local_gnn_type={self.local_gnn_type}, ' \
            f'global_model_type={self.global_model_type}, ' \
            f'heads={self.num_heads} (LoRA enabled)'
        return s
