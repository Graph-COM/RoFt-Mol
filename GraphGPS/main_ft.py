...
def custom_set_out_dir(cfg, cfg_fname, name_tag, fewshot, fewshot_num, reg_coef, split, surg):
    ...
    if 'surg' in cfg.model.loss_fun:
        run_name = "FT" + "_" + cfg.dataset.name + "_" + split + "_" + "Fewshot_" + str(fewshot) + "_" + str(fewshot_num) + "_"+ str(layer) + "_type_" + str(cfg.model.loss_fun) + "_tune_" + surg
...
if __name__ == '__main__':
    ...
    args.opts = []
    load_cfg(cfg, args)
    custom_set_out_dir(cfg, args.cfg_file, cfg.name_tag, fewshot, fewshot_num, reg_coef, split, surg)
    dump_cfg(cfg)
...
# Repeat for multiple experiment runs
    for run_id, seed, split_index in zip(*run_loop_settings()):
        ...
        loggers = create_logger()
        if 'surg' in cfg.model.loss_fun:
            # cfg.model.loss_fun = "filter_bce" if cfg.dataset.task_type == 'classification' else 'mse'
            import re
            # Compile a regex that captures the layer index
            ...
            for name, param in model.named_parameters():
                ...
                if m:
                    ...
                    # Enable grad for layers 0 through 5 (inclusive)
                    param.requires_grad = start <= layer_id <= end
                else:
                    # Disable grad for any parameter not in model.layers.<n>
                    param.requires_grad = False
        ...
        # Then enable only LoRA adapter weights:
        for name, param in model.named_parameters():
            if 'lora_' in name:
        ...
        if 'lpft' in cfg.model.loss_fun:
            # cfg.model.loss_fun = "filter_bce" if cfg.dataset.task_type == 'classification' else 'mse'
            for key, param in model.named_parameters():
                param.requires_grad = True
            optimizer_lp = create_optimizer(model.parameters(),
                                        new_optimizer_config(cfg))
            scheduler_lp = create_scheduler(optimizer_lp, new_scheduler_config(cfg))
            train_dict[cfg.train.mode](loggers, loaders, model, model_pt, reg_coef, optimizer_lp,
                                       scheduler_lp)
