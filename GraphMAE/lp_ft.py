...
def linear_probing(args, model, device, train_loader, optimizer_cls, val_loader, test_loader, writer):
    ...
    val_roc_list = []
    test_roc_list = []
    best_val_roc = 100 if args.regression else -1
    best_cls = output_layer.state_dict()
    best_func = min if args.regression else max

    for epoch in range(1, args.epochs + 1):
        loss_acc = train_func(model, device, train_loader, optimizer_cls)
        ...
        bol_update = (val_roc < best_val_roc) if args.regression else (val_roc > best_val_roc)
        
        if bol_update:
            best_val_roc = val_roc
            best_cls = output_layer.state_dict()

    print('Best epoch:', val_roc_list.index(best_func(val_roc_list)))
    print('Best auc: ', test_roc_list[val_roc_list.index(best_func(val_roc_list))])
...
if __name__ == '__main__':
    ...
    for seed in allseed:
        ...
        logs = 'Dataset:{}, Split:{}, Fewshot_{}_{}, Seed:{}, LP-FT, Best Epoch:{}, Best Acc:{:.5f}'.format(args.dataset, args.split, args.fewshot, args.fewshot_num, args.runseed, val_roc_list.index(best_func(val_roc_list)), test_roc_list[val_roc_list.index(best_func(val_roc_list))])
