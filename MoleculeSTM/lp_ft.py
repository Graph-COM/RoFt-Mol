...
def train(args, epoch, model, device, loader, optimizer):
    ...
    for step, batch in enumerate(epoch_iter):
        ...
        graph_rep, pred = model(batch.x, batch.edge_index, batch.edge_attr, batch.batch)
...
def linear_probing(args, model, device, train_loader, optimizer_cls, val_loader, test_loader, writer):
    train_acc_list = []
    val_acc_list = []
    test_acc_list = []
    best_val_acc = 100 if args.regression else -1
    best_cls = model.graph_pred_linear.state_dict()
    best_func = min if args.regression else max

    for epoch in range(1, args.epochs+1):
        print("====LP epoch " + str(epoch))
        
        train(args, epoch, model, device, train_loader, optimizer_cls)
...
def main():
    ...
    for seed in allseed:
        ...
        state_dict = torch.load('molecule_model.pth')
        state_dict.pop('graph_pred_linear.bias')
        state_dict.pop('graph_pred_linear.weight')
        new_dict = OrderedDict((key.split(".", 1)[1], value) for key, value in state_dict.items())
        model.molecule_node_model.load_state_dict(new_dict)
        model.to(device)
        ...
        optimizer = optim.Adam(list(model.molecule_node_model.parameters()) + list(model.graph_pred_linear.parameters()), lr=args.lr, weight_decay=args.decay)
        optimizer_cls = optim.Adam(model.graph_pred_linear.parameters(), lr=args.lr, weight_decay=args.decay)
        ...
        best_cls = linear_probing(args, model, device, train_loader, optimizer_cls, val_loader, test_loader, writer)
        model.graph_pred_linear.load_state_dict(best_cls)
        ...
        exp_path = os.getcwd() + '/{}_results/{}/'.format("MoleculeSTM", args.dataset)
